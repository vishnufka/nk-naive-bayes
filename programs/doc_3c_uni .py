...
#Experiment for documents, 3 classes, unigrams only.
#91% accuracy, 114 minutes to train
...
list_train = list_pos_tagged[29:] + list_neu_tagged[34:] + list_neg_tagged[37:]#size = 300
list_test = list_pos_tagged[:29] + list_neu_tagged[:34] + list_neg_tagged[:37]#size = 100
...
for entry in list_train:
    list_train_unigrams += [({word: (word in word_tokenize(entry[0])) for word in set_train_unigrams}, entry[1])]

for entry in list_test:
    list_test_unigrams += [({word: (word in word_tokenize(entry[0])) for word in set_test_unigrams}, entry[1])]
...
with open("/home/s/Project/experiments/experiment_python_programs/classifiers/doc_3c_uni_classifier.pickle", "wb") as f:
